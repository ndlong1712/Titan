...
//    }
    ...
    [self updateSelectionInfoWithAnimated:NO];
    
    if (self.imagePickerController.allowsImageSelectMultipleTimes) {
...
- (void)updateSelectionInfoWithAnimated:(BOOL)animated {
    NSMutableArray *selectedAssets = self.imagePickerController.selectedAssets;
    
    //show animation
    UIView *customView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 45)];
...
//    }
    ...
    [(UIBarButtonItem *)self.toolbarItems[1] setCustomView:customView];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.showsOverlayViewWhenSelected = self.imagePickerController.allowsMultipleSelection && !self.imagePickerController.allowsImageSelectMultipleTimes;
    ...
    if (kind == UICollectionElementKindSectionFooter) {
        ...
        switch (self.imagePickerController.mediaType) {
            ...
            case QBImagePickerMediaTypeImage:
            {
                ...
                if (numberOfPhotos == 0) {
                    numberOfPhotos = self.assetCollection.estimatedAssetCount;
                }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSMutableArray *selectedAssets = imagePickerController.selectedAssets;
    ...
    if (imagePickerController.allowsMultipleSelection) {
        ...
        if (imagePickerController.showsNumberOfSelectedAssets) {
            [self updateSelectionInfoWithAnimated:YES];
...
#pragma mark - Handle tap on UICollectionView


- (void)collectionViewTapped:(UIGestureRecognizer *)gesture {
    
    CGPoint tappedPoint = [gesture locationInView:self.collectionView];
    
    NSIndexPath *indexPath = [self.collectionView indexPathForItemAtPoint:tappedPoint];
    
    if (indexPath && !self.touching) {
        QBImagePickerController *imagePickerController = self.imagePickerController;
        NSMutableArray *selectedAssets = imagePickerController.selectedAssets;
        ...
        if ([self.imagePickerController.delegate respondsToSelector:@selector(qb_imagePickerController:asset:completion:)]) {
            [self.imagePickerController.delegate qb_imagePickerController:self.imagePickerController asset: asset completion:^(BOOL shouldSelect) {
                self.touching = NO;
                if (!shouldSelect) {
                    return;
                }
            ...
            //            shouldSelect = [self.imagePickerController.delegate qb_imagePickerController:self.imagePickerController shouldSelectAsset:asset];
        }
...
{
    NSUInteger numberOfColumns = self.imagePickerController.numberOfColumnsInLandscape;
    float widthScren = 0;
    ...
    if ( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad )
    {
        if (orientation == UIDeviceOrientationPortrait) {
            widthScren = [[UIScreen mainScreen] bounds].size.width;
        } else {
            widthScren = [[UIScreen mainScreen] bounds].size.height;
        }
    } else {
        widthScren = [[UIScreen mainScreen] bounds].size.width;
    }
    
    CGFloat width = (widthScren - 2.0 * (numberOfColumns - 1)) / numberOfColumns;
